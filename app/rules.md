...
|---|---|---|
| Vídeos | `screens/videos.html` | Listagem de vídeos vinculados ao YouTube (2026-08-05, "Jornada · Vídeos"), acessada pelo item "Vídeos" da Sidebar. Grid `auto-fill` de cards (thumbnail + badge de categoria + título + CTA "Assistir no YouTube"), sempre ordenado do mais recente pro mais antigo. Card inteiro clicável (`role="link"`/`tabindex`), abre a URL em nova aba via `window.open`, nunca player embutido. Sem tela de cadastro/admin nesta rodada — `videos-data.js` (`window.NiveloVideos`) já implementa validação de link + extração de metadados via oEmbed público do YouTube (`add()`), pronto pra uma futura tela administrativa. Ícone do YouTube é SVG inline (não Lucide — `data-lucide="youtube"` não existe na build servida pelo CDN, corrigido no round 71) |
| Minha Conta | `screens/minha-conta.html` | Área de conta do usuário (2026-08-05, "Jornada · Minha Conta"), 3 abas via `Tab` real (Dados/Plano/Pagamento), hash combinado `#tab=X&state=Y` pros cenários de demonstração (mesmo mecanismo de `#step=N&state=X` de `nova-fazenda.html`). Dados: campos cadastrais completos + endereço com CEP/ViaCEP + seção reservada pra dados extras de cobrança (Cielo). Plano: card de status + seletor "Escolher outro plano" (toggle Mensal/Anual + 4 cards) — qualquer troca abre um modal "Falar com Comercial" (WhatsApp), nunca upgrade automático. Pagamento: renovar/cancelar renovação automática/avisos de vencimento/histórico de compras. Dados: `planos-data.js`/`assinatura-data.js`/`minha-conta-data.js` |
| Contratar plano | `screens/comprar-plano.html` | Fluxo de checkout completo (2026-08-05, redesenhado no round 72), chrome minimalista sem Sidebar (só logo+"Cancelar e voltar"). **2 etapas** (Plano → Pagamento, não 3 — Modalidade foi absorvida pela Etapa 1 no round 72): Plano é um ACORDEÃO vertical (`.cp-plan-accordion`, nunca cards de landing page lado a lado) — clicar num plano expande in-place mostrando recursos + as 2 opções de modalidade (Mensal/Anual, com preço/economia) + "Continuar" escopado àquele plano. Pagamento: Cartão sempre visível/editável; PIX só quando Anual. Cartão: máscara de número/validade/CVV (nova no projeto) + parcelamento 1-12x sem juros, empilhado verticalmente no dropdown (só Anual+Cartão). PIX: QR ilustrativo (SVG determinístico 21×21) + código copia-e-cola + mock de verificação em 2 cliques. Cupom (`NIVELO10`/`NIVELO20`) + resumo ao vivo. Cartão terminado em `0002` simula recusa (mesmo espírito do OTP fixo do Login). Confirmação com ícone de sucesso + dados da assinatura + "Ir para o Dashboard". Acessível de `dashboard.html` ("Contratar agora"/"Realizar pagamento") — mesma tela nos 2 casos, `?motivo=renovacao\|vencido` só ajusta um banner de contexto. **3 bugs reais corrigidos no round 72** (`RadioButton.module.css` colidindo com `Input`/`Dropdown`, mesmo padrão de sempre): inputs de texto invisíveis, parcelamento agrupado horizontal, radio de método sem indicação visual de selecionado — ver `page-comprar-plano.css` pros guards |
