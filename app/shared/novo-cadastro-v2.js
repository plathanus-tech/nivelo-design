(function () {
  ...
  // ---------- Nome (declarado cedo, ANTES do Tipo/Transportadora Própria
  // logo abaixo) ----------
  // V2 precisa de `nameField`/`setNameError` disponíveis já na primeira
  // seleção de Tipo (`tipoDropdown.selectValues(['cliente'])`, mais abaixo),
  // que dispara `refreshTransportadoraField()` → `applyPropriaSimplification()`
  // no mesmo tick — por isso este bloco (só as 2 linhas de validação de
  // Nome, não o resto de `runValidation()`) subiu pra antes do Tipo, em vez
  // de continuar junto do resto da validação no final do arquivo.
  ...
  // ---------- V2: "Tipo de transportadora" (Própria/Terceira) ----------
  // Declarado ANTES do dropdown de Tipo, propositalmente: `tipoDropdown.
  // selectValues([...])` dispara o `onChange` de forma SÍNCRONA já na
  // configuração inicial (ver abaixo), e esse callback chama
  // `refreshTransportadoraField()` — com `var`, se este bloco viesse depois,
  // `tipoTransportadoraField` ainda seria `undefined` nesse primeiro disparo
  // (hoisting: a declaração `var` sobe, a atribuição não), quebrando o
  // script inteiro com "Cannot set properties of undefined".
  //
  // Só faz sentido perguntar isso quando Transportadora é o ÚNICO tipo
  // marcado — se a Pessoa/Empresa também é Cliente/Fornecedor, ela exerce
  // múltiplos papéis e o cadastro completo (dados/endereço) continua
  // necessário pros outros papéis, então "Própria" deixa de fazer sentido
  // como conceito aqui (ver pedido: "não considerar como transportadora
  // própria" quando há mais de um tipo selecionado).
  var tipoTransportadoraField = document.getElementById('tipo-transportadora-field');
  var fiscalSectionTitle = document.getElementById('fiscal-section-title');
  var addressSection = document.getElementById('address-section');
  var isPropriaSimplified = false;

  var tipoTransportadoraDropdown = initDropdown(tipoTransportadoraField, function (value) {
    applyPropriaSimplification(value === 'propria');
  });

  function isOnlyTransportadora(values) {
    return values.length === 1 && values[0] === 'transportadora';
  }

  // Chamado sempre que o Tipo muda: mostra/esconde o campo "Tipo de
  // transportadora" e, se ele deixar de fazer sentido (Transportadora +
  // outro tipo, ou Transportadora removida), desfaz a simplificação e limpa
  // a seleção — nunca deixa o formulário "preso" no modo enxuto por engano.
  function refreshTransportadoraField(values) {
    var shouldShow = isOnlyTransportadora(values);
    tipoTransportadoraField.hidden = !shouldShow;
    if (!shouldShow) {
      tipoTransportadoraDropdown.root.classList.remove('open');
      delete tipoTransportadoraField.dataset.value;
      var valueEl = tipoTransportadoraField.querySelector('[data-dropdown-value]');
      valueEl.textContent = 'Selecione';
      valueEl.classList.add('placeholder');
      Array.prototype.slice.call(tipoTransportadoraField.querySelectorAll('.option')).forEach(function (o) { o.classList.remove('selected'); });
      applyPropriaSimplification(false);
    }
  }

  // Alterna entre o formulário completo e o formulário enxuto de
  // Transportadora Própria: esconde Nome/Tipo de pessoa/CPF-CNPJ/Contato/
  // Endereço (a própria empresa já tem esses dados em outro lugar do
  // sistema, ex. Minha Conta/Emitente — pedir de novo aqui só repetiria
  // trabalho) e reduz "Informações fiscais e contato" a só os 2 primeiros
  // campos fiscais (Inscrição estadual/Contribuinte), renomeando o título
  // pra "Informações fiscais". Cadastro de veículos continua igual —
  // registrar a frota da própria transportadora é justamente o motivo de
  // cadastrá-la.
  function applyPropriaSimplification(enabled) {
    isPropriaSimplified = enabled;
    nameField.hidden = enabled;
    document.getElementById('fantasy-field').hidden = enabled;
    document.getElementById('pessoa-tipo-field').hidden = enabled;
    document.getElementById('document-field').hidden = enabled;
    document.getElementById('phone-field').hidden = enabled;
    document.getElementById('email-field').hidden = enabled;
    addressSection.hidden = enabled;
    fiscalSectionTitle.textContent = enabled ? 'Informações fiscais' : 'Informações fiscais e contato';
    if (enabled) setNameError(false);
  }
  ...
  var tipoDropdown = initMultiDropdown(document.getElementById('tipo-field'), function (values) {
    ...
    refreshTransportadoraField(values);
  });
  ...
  // `nameField`/`nameInput`/`setNameError` já foram declarados mais acima
  // (ver comentário perto do Tipo/Código) — precisavam existir antes da
  // primeira seleção de Tipo, que já dispara a lógica de Transportadora
  // Própria/Terceira.
  ...
  function runValidation() {
    // Transportadora Própria: Nome fica escondido/fora do fluxo, então não
    // faz sentido exigi-lo (nem mostrar a borda vermelha num campo que o
    // usuário nem vê).
    var hasError = !isPropriaSimplified && !nameInput.value.trim();
  ...
  // ---------- V2: estados de demonstração dos 5 cenários principais desta
  // variante (ver prototype-nav) ----------
  // `transportadora` (já existia acima) cobre o cenário 2 (Somente
  // Transportadora, "Tipo de transportadora" aparece ainda sem escolha).
  if (demoState === 'transportadoraoutro') {
    // Cenário 3: Transportadora + outro tipo → nunca é "própria", cadastro
    // normal/completo, campo "Tipo de transportadora" nem aparece.
    tipoDropdown.selectValues(['transportadora', 'fornecedor']);
  }
  if (demoState === 'propria') {
    // Cenário 4: Transportadora + Própria → formulário simplificado.
    tipoDropdown.selectValues(['transportadora']);
    addVehicle();
    var propriaOption = tipoTransportadoraField.querySelector('.option[data-value="propria"]');
    if (propriaOption) tipoTransportadoraDropdown.selectOption(propriaOption);
  }
  if (demoState === 'terceira') {
    // Cenário 5: Transportadora + Terceira → formulário completo.
    tipoDropdown.selectValues(['transportadora']);
    addVehicle();
    var terceiraOption = tipoTransportadoraField.querySelector('.option[data-value="terceira"]');
    if (terceiraOption) tipoTransportadoraDropdown.selectOption(terceiraOption);
  }
