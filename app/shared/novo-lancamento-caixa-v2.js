(function () {
  ...
  // Data de referência fixa do protótipo (mesma constante já usada em
  // contas-pagar-data.js/dre.js/etc. — nunca `new Date()`).
  ...
  var documentoInput = document.getElementById('nlc-documento');

  // Contas financeiras (Configuração > Conta Financeira): fonte real do
  // dropdown, mesmo catálogo usado na aba "Contas financeiras" do Caixa V2.
  ...
  // ---------- Data: preenchida automaticamente com TODAY, usuário pode
  // alterar. ----------
  ...
  dataPicker.setValue(TODAY);

  // ---------- Resumo financeiro do lançamento — recalcula sozinho conforme
  // Conta financeira / Tipo / Valor mudam. ----------
  var resumoSaldoAtualEl = document.getElementById('nlc-resumo-saldo-atual');
  var resumoMovimentoLabelEl = document.getElementById('nlc-resumo-movimento-label');
  var resumoMovimentoEl = document.getElementById('nlc-resumo-movimento');
  var resumoNovoSaldoEl = document.getElementById('nlc-resumo-novo-saldo');
  var resumoTipoEl = document.getElementById('nlc-resumo-tipo');

  function updateResumo() {
    var contaCodigo = contaFinanceiraField.dataset.value;
    var tipo = tipoField.dataset.value;
    var valor = Number(valorInput.dataset.cents || 0) / 100;

    if (!contaCodigo) {
      resumoSaldoAtualEl.textContent = '—';
      resumoNovoSaldoEl.textContent = '—';
      resumoNovoSaldoEl.classList.remove('caixa-valor-entrada', 'caixa-valor-saida');
    } else {
      var saldoAtual = window.NiveloCaixa.saldoPorContaFinanceira(contaCodigo);
      resumoSaldoAtualEl.textContent = formatMoedaSinal(saldoAtual);

      var novoSaldo = saldoAtual;
      if (tipo === 'entrada') novoSaldo = saldoAtual + valor;
      else if (tipo === 'saida') novoSaldo = saldoAtual - valor;
    ...
    if (!tipo) {
      resumoMovimentoLabelEl.textContent = 'Entrada ou Saída';
      resumoMovimentoEl.textContent = '—';
      resumoMovimentoEl.classList.remove('caixa-valor-entrada', 'caixa-valor-saida');
      resumoTipoEl.textContent = '—';
    } else {
      var isEntrada = tipo === 'entrada';
      resumoMovimentoLabelEl.textContent = isEntrada ? 'Entrada' : 'Saída';
      resumoMovimentoEl.textContent = formatMoedaSinal(isEntrada ? valor : -valor);
      resumoMovimentoEl.classList.toggle('caixa-valor-entrada', isEntrada);
      resumoMovimentoEl.classList.toggle('caixa-valor-saida', !isEntrada);
      resumoTipoEl.textContent = isEntrada ? 'Entrada' : 'Saída';
    }
  }
  updateResumo();
  ...
  function runValidation() {
    ...
    return !contaFinanceiraInvalid && !tipoInvalid && !dataInvalid && !valorInvalid && !historicoInvalid && !categoriaInvalid;
  }
  ...
  form.addEventListener('submit', function (event) {
    ...
    if (!runValidation()) {
      var firstInvalid = form.querySelector('.wrapper.error, .dpRoot.error');
      if (firstInvalid) {
    ...
    var contaFinanceira = window.NiveloContasFinanceiras.findByCodigo(contaFinanceiraField.dataset.value);
    ...
    var payload = {
      ...
      banco: contaFinanceira ? contaFinanceira.nome : '',
