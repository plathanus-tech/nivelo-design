(function () {
  ...
  function showErrorToast(title, message) {
    var toast = document.createElement('div');
    toast.className = 'alert error nf-toast';
    toast.setAttribute('role', 'status');
    toast.innerHTML =
      '<span class="icon"><i data-lucide="circle-x" width="18" height="18"></i></span>' +
      '<div class="body">' +
      '<div class="title">' + title + '</div>' +
      '<div class="message">' + message + '</div>' +
      '</div>' +
  ...
  // Ícone do WhatsApp: mesmo SVG inline (fill="currentColor") já usado no
  // item "Suporte" da Sidebar e em "Configurar WhatsApp" — exceção já
  // documentada à regra "sem SVG inline" (a marca não existe no Lucide).
  var WHATSAPP_ICON_SVG = '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
  ...
  function buildActionsHTML(nota) {
    var html = '<div class="cellActions">' +
      ...
      '<button type="button" class="actionBtn" data-action="whatsapp" aria-label="Enviar para WhatsApp">' + WHATSAPP_ICON_SVG + '<span class="tip text-body-xs top"><span class="arrow"></span>Enviar para WhatsApp</span></button>';
  ...
  // Bug real corrigido: `onApply` só atualizava `state.periodStart`/`-End`,
  // sem nunca chamar `applyFilters()` — o filtro só passava a valer de
  // verdade se o usuário, depois de aplicar o período (fechando o popover
  // PRÓPRIO do período), reabrisse o Agrupamento de Filtros e clicasse no
  // "Aplicar" DE FORA de novo. Na prática (e agravado por um 2º bug: o
  // clique dentro do popover do período, anexado a `document.body` fora da
  // árvore de `filtrosPopoverEl`, já fecha o Agrupamento de Filtros sozinho
  // via `outsideFiltrosClickHandler`), o usuário selecionava um período,
  // clicava em "Aplicar" e via a tabela continuar exatamente igual — lido
  // como "o filtro de período não funciona". Corrigido chamando
  // `applyFilters()` direto no `onApply`, mesmo padrão já usado em Pedidos
  // de Venda: mudar o período já atualiza a tabela na hora, sem depender
  // de um 2º clique em "Aplicar" nem da ordem de fechamento dos popovers.
  ...
  // ---------- Enviar para WhatsApp: usa SEMPRE a mesma conexão já
  // configurada em Assistente IA > Configurar WhatsApp (`window.
  // NiveloWhatsappNumeros`) — nunca solicita/duplica uma conexão própria
  // pra esta ação. O destinatário é resolvido a partir do cliente/
  // fornecedor já vinculado à nota (`window.NiveloCadastros`, mesmo
  // catálogo já usado em todo o sistema — casado pelo documento, único
  // dado em comum entre os dois módulos), nunca um número digitado na
  // hora. Sem backend real neste protótipo: o "envio" é simulado (mock de
  // latência), sempre terminando em sucesso ou falha clara, nunca
  // silencioso. ----------
  function resolveContato(nota) {
    var nome = nota.tipo === 'saida' ? nota.clienteNome : nota.fornecedorNome;
    var documento = nota.tipo === 'saida' ? nota.clienteDocumento : nota.fornecedorDocumento;
    var cadastro = window.NiveloCadastros
      ? window.NiveloCadastros.list().filter(function (c) { return c.documento === documento; })[0]
      : null;
    return { nome: nome, telefone: cadastro ? cadastro.telefone : null };
  }

  function enviarWhatsApp(numero, btn) {
    var nota = window.NiveloNotasFiscais.findByNumero(numero);
    if (!nota) return;

    if (!window.NiveloWhatsappNumeros || !window.NiveloWhatsappNumeros.isConnected()) {
      showErrorToast('Não foi possível enviar por WhatsApp.', 'Nenhum WhatsApp está conectado ao Assistente de IA. Conecte um número em Assistente IA → Configurar WhatsApp e tente novamente.');
      return;
    }

    var contato = resolveContato(nota);
    if (!contato.telefone) {
      showErrorToast('Não foi possível enviar por WhatsApp.', 'Não há telefone cadastrado para ' + (contato.nome || 'este contato') + '. Cadastre um telefone em Cadastro → Parceiros e tente novamente.');
      return;
    }

    // Mock de latência de envio (sem backend real) — botão fica desabilitado
    // brevemente, mesmo espírito já usado em outros flash-disable do sistema.
    if (btn) btn.disabled = true;
    window.setTimeout(function () {
      if (btn) btn.disabled = false;
      showSuccessToast(
        'Nota fiscal enviada por WhatsApp com sucesso.',
        'NF ' + nota.numero + ' enviada para ' + contato.nome + ' (' + window.NiveloWhatsappNumeros.formatNumero(contato.telefone) + ') pelo WhatsApp conectado ao Assistente de IA.'
      );
    }, 600);
  }
  ...
  function handleRowAction(btn, row) {
    ...
    else if (action === 'whatsapp') enviarWhatsApp(numero, btn);
  }
