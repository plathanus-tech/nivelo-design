(function () {
  ...
  var DRAFT_KEY = 'nivelo.novacontareceber.rascunho';
  var CLIENTE_CRIADO_KEY = 'nivelo.novacontareceber.cliente-criado';
  ...
  function goToNovoCliente() {
    ...
    window.location.href = 'novo-cadastro.html?tipo=cliente&return=nova-conta-receber';
  }
  ...
  var editingConta = codigoParam ? window.NiveloContasReceber.findByCodigo(codigoParam) : null;
  ...
  document.getElementById('cancelar-conta-confirmar').addEventListener('click', function () {
    window.NiveloContasReceber.cancelar(editingConta.codigo);
    try { sessionStorage.setItem('nivelo.novacontareceber.success', 'Conta a receber cancelada com sucesso.'); } catch (e) {}
    window.location.href = 'contas-a-receber.html';
  });
  ...
  form.addEventListener('submit', function (event) {
    ...
    try {
      if (editingConta) {
        window.NiveloContasReceber.update(editingConta.codigo, payload);
        sessionStorage.setItem('nivelo.novacontareceber.success', 'Conta a receber atualizada com sucesso.');
      } else {
        var criados = window.NiveloContasReceber.add(payload);
        var mensagem = criados.length > 1
          ? criados.length + ' lançamentos gerados com sucesso.'
          : 'Conta a receber salva com sucesso.';
        sessionStorage.setItem('nivelo.novacontareceber.success', mensagem);
      }
    } catch (e) {}
