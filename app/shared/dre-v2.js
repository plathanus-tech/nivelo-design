(function () {
  ...
  // ---------- Classificação da DRE (idêntica à V1, ver app/CLAUDE.md pra
  // histórico completo da validação de UX que definiu esta estrutura) ----------
  ...
  // ---------- Dropdown genérico (Baseado em) — mesmo initDropdown() simples já
  // usado em outras telas (ex.: Agrupamento na V1), reaproveitado aqui pro novo
  // campo "Baseado em". ----------
  ...
  // ---------- Filtro "Baseado em" — NOTA IMPORTANTE: seletor UI-only,
  // reservado pra uma futura integração ainda não modelada neste protótipo.
  // Não existe hoje nenhum vínculo real entre window.NiveloCaixa (a fonte de
  // dados que já alimenta a V1 do DRE) e window.NiveloNotasFiscais/um futuro
  // módulo de "Pedidos de vendas" (que nem existe como entidade própria neste
  // sistema). As 2 opções do dropdown abaixo, portanto, disparam EXATAMENTE o
  // mesmo cálculo (agregação de NiveloCaixa) — a escolha só é armazenada em
  // estado e refletida no resumo dos filtros, pronta pra quando essa
  // integração de dados for construída de verdade. Mesmo espírito de outros
  // stubs "preparados, não ligados a dado real" já documentados no projeto
  // (Conta Financeira antes do round 59, Certificado Digital > Parceiro). ----------
  var baseadoEmDropdown = initDropdown(document.getElementById('baseado-em-field'));
  var BASEADO_EM_LABEL = { 'notas-fiscais': 'Notas Fiscais', 'pedidos-vendas': 'Pedidos de vendas' };
  function getBaseadoEmValue() { return document.getElementById('baseado-em-field').dataset.value || 'notas-fiscais'; }

  // ---------- Ano (período sempre um ano-calendário inteiro: 01/jan a
  // 31/dez do ano escolhido) — NiveloDatePicker.initYear(), mesmo padrão
  // visual do seletor de mês/ano (Competência), só que navegando por
  // década/selecionando um ano inteiro por clique. Só anos ANTERIORES ao
  // ano corrente do sistema podem ser selecionados (o ano atual nunca é
  // uma opção de pesquisa, conforme pedido) — `maxYear` desabilita os
  // botões de ano fora do intervalo permitido no próprio grid. ----------
  // Nota pra desenvolvimento: mesma regra do LCDPR V2 — a lista de anos deve ser
  // dinâmica, derivada da data de início de uso do sistema pela empresa até o ano
  // anterior ao corrente, nunca incluindo o ano atual/futuro. Anos fora desse
  // intervalo são removidos da lista (não só desabilitados). Hardcoded aqui pra
  // fins de protótipo, com o mesmo intervalo fictício já usado no LCDPR V2.
  var ANO_CORRENTE_SISTEMA = 2026;
  var ANO_INICIO_USO_SISTEMA = 2023;
  var ANOS_DISPONIVEIS_DRE = [];
  for (var anoDreDisponivel = ANO_INICIO_USO_SISTEMA; anoDreDisponivel < ANO_CORRENTE_SISTEMA; anoDreDisponivel++) {
    ANOS_DISPONIVEIS_DRE.push(anoDreDisponivel);
  }
  var anoPicker = window.NiveloDatePicker.initYear({
    rootId: 'ano-field',
    triggerId: 'dre-ano-trigger',
    valueId: 'dre-ano-value',
    popoverId: 'dre-ano-popover',
    placeholder: 'Selecionar ano',
    allowedYears: ANOS_DISPONIVEIS_DRE,
    initialValue: ANO_CORRENTE_SISTEMA - 1
  });

  // ---------- Montagem das colunas (sempre 12 meses do ano escolhido) ----------
  // Agrupamento removido junto com o filtro correspondente — como o período é
  // sempre um ano-calendário inteiro, "Mensal" (12 colunas) é o encaixe
  // natural pra uma visão anual de DRE; a heurística automática Diário/
  // Semanal/Mensal da V1 (chooseAgrupamento) não faz sentido aqui e foi
  // removida junto.
  function buildColumns(ano) {
    var columns = [];
    for (var m = 1; m <= 12; m++) {
      columns.push({ key: ano + '-' + pad2(m), label: MONTH_ABBR[m - 1] });
    }
    return columns;
  }

  function columnIndexFor(dataIso, columns) {
  ...
  function gerarRelatorio() {
    ...
    if (!ano) { showToast('Informe o ano', 'Selecione o ano para gerar o relatório.'); return; }

    var start = ano + '-01-01';
    var end = ano + '-12-31';
    var columns = buildColumns(ano);
    var baseadoEm = getBaseadoEmValue();
    ...
    // Categoria removida como filtro — o relatório sempre inclui TODAS as
    // categorias ativas que entram no DRE (equivalente a "Todas as
    // categorias" sempre selecionado na V1), nunca restringindo por
    // `categoriaCodigo` antes da classificação em buckets.
    ...
    // "Baseado em" ainda não tem 2 fontes de dado reais (ver comentário acima
    // de `baseadoEmDropdown`) — as 2 opções agregam a MESMA base (NiveloCaixa).
    window.NiveloCaixa.list().forEach(function (l) {
      if (l.data < start || l.data > end) return;
      var idx = columnIndexFor(l.data, columns);
      if (idx === -1) return;
    ...
    // Cascata contábil idêntica à V1 (Receita Bruta → Deduções → Receita
    // Líquida → Custos → Lucro Bruto → Despesas Operacionais → Resultado
    // Operacional → Outras Receitas/Despesas → Resultado Financeiro →
    // Resultado do Exercício) — comportamento/matemática 1:1 preservados.
    ...
    // Derivado (não somado bucket a bucket): garante por construção que
    // Receita Bruta − Total de Despesas = Resultado do Período, mesmo com
    // Outras Receitas entrando na cascata como uma ADIÇÃO (mesmo invariante
    // já validado na V1).
    var grandDespesas = grandReceita - grandResultado;
    var margem = grandReceita !== 0 ? (grandResultado / grandReceita * 100) : null;

    currentReport = {
      ano: ano, start: start, end: end,
      columns: columns,
      ...
      baseadoEmLabel: BASEADO_EM_LABEL[baseadoEm] || BASEADO_EM_LABEL['notas-fiscais']
    };
  ...
  function renderReport(report) {
    ...
    document.getElementById('dre-resumo-filtros').textContent =
      'Ano: ' + report.ano + ' · Baseado em: ' + report.baseadoEmLabel;
  ...
  // ---------- Eixo X — sempre 12 colunas mensais (nunca mais que 12
  // rótulos), então nenhuma amostragem/labelStep é necessária: todo mês
  // aparece. ----------
  function buildAxisLabelsHTML(report, labelClass) {
    return report.columns.map(function (c) {
      return '<span class="' + labelClass + ' text-10-medium">' + c.label + '</span>';
    }).join('');
  }

  // ---------- Gráfico de barras (SVG puro) — barra ÚNICA divergente
  // (Resultado por mês): sobe em verde quando positiva, desce em vermelho
  // quando negativa, a partir de uma linha de base calculada pela
  // proporção entre o maior valor positivo e o maior valor negativo do
  // ano. Idêntico à lógica da V1, só que sempre com 12 colunas. ----------
  ...
  function showChartTooltip(clientX, clientY, idx) {
    ...
    chartTooltip.innerHTML =
      '<div class="dre-chart-tooltip-header">Período: ' + col.label + '</div>' +
  ...
  // ---------- Cross-highlight gráfico ↔ tabela (idêntico à V1) ----------
  ...
  // ---------- Tabela hierárquica (grupos expansíveis) — idêntica à V1 ----------
  var collapsedGroups = {};
  ...
  // Cascata em ordem (idêntica à V1): Receita Bruta → categorias → Total da
  // Receita Bruta → (-) Deduções da Receita [Devoluções/Impostos, só quando
  // há valor] → Receita Líquida → (-) Custos → Lucro Bruto → (-) Despesas
  // Operacionais → Resultado Operacional → (+) Outras Receitas → (-) Outras
  // Despesas → (+/-) Resultado Financeiro → (=) Resultado do Exercício.
  ...
  // ---------- Rolagem horizontal da tabela (mesma técnica da V1/Balancete/LCDPR) ----------
  ...
  // ---------- Gráfico: card colapsável, preferência persistida (mesma
  // chave/raciocínio da V1: nasce expandido na 1ª geração, lembra a escolha
  // do usuário entre gerações/reloads seguintes via localStorage). ----------
  ...
  // mesmo padrão de flash-disable da V1/Balancete). ----
