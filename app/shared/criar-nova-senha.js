(function () {
  ...
  var form = document.getElementById('newpass-form');
  var newpassField = document.getElementById('newpass-field');
  var newpassInput = document.getElementById('new-password');
  ...
  var submitBtn = document.getElementById('newpass-submit');
  var sameAsOldBanner = document.getElementById('same-as-old-banner');
  var saveErrorBanner = document.getElementById('save-error-banner');
  ...
  // protótipo (sem backend), então sempre retorna false. A checagem já
  // fica plugada no fluxo de validação pra quando o backend informar isso.
  ...
  wireToggle('toggle-new-password', newpassInput);
  wireToggle('toggle-confirm-password', confirmInput);
  ...
  form.addEventListener('submit', function (event) {
    ...
    window.setTimeout(function () {
      try {
        sessionStorage.setItem('nivelo.recovery.success', '1');
      } catch (e) {}
      window.location.href = 'dashboard.html#state=success';
    }, 700);
  });
