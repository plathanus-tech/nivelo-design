// Catálogo central do Canal de Ideias (`window.NiveloIdeias`) — cópia de
// app/shared/ideias-data.js (mesma convenção IIFE, mesmo seed/regras de
// votos/comentários), com uma única adição: `remove(codigo)`, exclusiva do
// painel administrativo (o cliente não tem essa permissão).
window.NiveloIdeias = (function () {
  ...
  var VOTES_KEY = 'nivelo.admin.ideias.votos';
  ...
  (function loadRemoved() {
    try {
      var raw = sessionStorage.getItem(REMOVED_KEY);
      if (!raw) return;
      var removidos = JSON.parse(raw);
      for (var i = IDEIAS.length - 1; i >= 0; i--) {
        if (removidos.indexOf(IDEIAS[i].codigo) !== -1) IDEIAS.splice(i, 1);
      }
    } catch (e) {}
  })();
  ...
  function persistRemoved(codigo) {
    try {
      var raw = sessionStorage.getItem(REMOVED_KEY);
  ...
  // Remove de vez (não é soft delete, o pedido não menciona preservar
  // histórico de ideia excluída) — persiste em sessionStorage pra não
  // reaparecer se o admin navegar entre as telas desta jornada.
  function remove(codigo) {
    var index = IDEIAS.findIndex(function (i) { return i.codigo === codigo; });
    if (index === -1) return false;
    IDEIAS.splice(index, 1);
    persistRemoved(codigo);
    return true;
  }
