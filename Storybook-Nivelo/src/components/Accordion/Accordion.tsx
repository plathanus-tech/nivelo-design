    ...
    <div className={styles.accordion}>
      {items.map((item, i) => (
        <div key={i} className={[styles.item, openIndexes.has(i) ? styles.open : ''].join(' ')}>
          <button className={`${styles.trigger} text-subtitle-m`} onClick={() => toggle(i)} aria-expanded={openIndexes.has(i)}>
            <span>{item.title}</span>
            <span className={styles.chevron}><ChevronDown size={16} /></span>
          </button>
          <div className={styles.panel} aria-hidden={!openIndexes.has(i)}>
            <div className={`${styles.content} text-body-m`}>{item.content}</div>
          </div>
        </div>
      ))}
    </div>
