      ...
      <div className={styles.list} role="tablist">
        {tabs.map((tab, i) => (
          <button key={i} role="tab" aria-selected={active === i} aria-controls={`tabpanel-${i}`}
            className={[styles.tab, 'text-subtitle-m', active === i ? styles.active : ''].filter(Boolean).join(' ')}
            onClick={() => select(i)}>
            {tab.label}
          </button>
        ))}
      </div>
      <div id={`tabpanel-${active}`} role="tabpanel" className={`${styles.panel} text-body-m`}>
        {tabs[active]?.content}
      </div>
    </div>
  );
