...
export function Input({ label, helperText, error, success, iconLeft, iconRight, hideErrorIcon = false, className, id, ...props }: InputProps) {
  ...
  return (
    <div className={[styles.wrapper, error ? styles.error : '', className ?? ''].filter(Boolean).join(' ')}>
      {label && <label className={`${styles.label} text-16-bold`} htmlFor={inputId}>{label}</label>}
      <div className={wrapClass}>
        {iconLeft && <span className={styles.iconLeft}>{iconLeft}</span>}
        <input id={inputId} className={`${styles.input} text-body-m`} {...props} />
        {iconRight && <span className={styles.iconRight}>{iconRight}</span>}
      </div>
      {error && (
        <span className={`${styles.errorText} text-body-xs`}>
          {!hideErrorIcon && <CircleX size={14} className={styles.msgIcon} />}
          {error}
        </span>
      )}
      {!error && success && (
        <span className={`${styles.successText} text-body-xs`}>
          <CircleCheck size={14} className={styles.msgIcon} />
          {success}
        </span>
      )}
      {!error && !success && helperText && <span className={`${styles.helperText} text-body-xs`}>{helperText}</span>}
    </div>
  );
}
