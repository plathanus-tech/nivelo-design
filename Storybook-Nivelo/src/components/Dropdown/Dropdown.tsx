...
export function Dropdown({ options, value, onChange, placeholder = 'Selecione...', disabled = false, label, error }: DropdownProps) {
  ...
  return (
    <div ref={ref} className={[styles.wrapper, open ? styles.open : '', error ? styles.hasError : ''].filter(Boolean).join(' ')}>
      {label && <span className={`${styles.label} text-16-bold`}>{label}</span>}
      <button className={`${styles.trigger} text-body-m`} onClick={() => !disabled && setOpen(v => !v)} disabled={disabled} type="button">
      ...
      <div className={styles.menu}>
        {options.map(opt => (
          <div key={opt.value} className={[styles.option, 'text-body-m', opt.value === value ? styles.selected : ''].filter(Boolean).join(' ')}
