...
export function StatusBadge({ status, label }: { status: StatusType; label: string }) {
  return (
    <span className={`${styles.badge} text-12-regular`} data-status={status}>
...
function MiniToggle({
  ...
  return (
    <label className={styles.miniToggle}>
      <input
        type="checkbox"
        className={styles.miniToggleInput}
        checked={checked}
    ...
    case 'link':
      return (
        <a
          ...
          className={`${styles.cellLink} text-12-bold`}
          onClick={e => {
    ...
    <div className={styles.card}>
      {(title || subtitle) && (
        <div className={styles.cardHeader}>
          {title    && <h2 className={`${styles.title} text-heading-6`}>{title}</h2>}
          {subtitle && <p  className={`${styles.subtitle} text-14-regular`}>{subtitle}</p>}
        </div>
      )}
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          <thead>
            <tr className={styles.headerRow}>
              {columns.map(col => (
                <th
                  key={String(col.key)}
                  className={[styles.th, 'text-12-bold', col.sortable ? styles.sortable : ''].filter(Boolean).join(' ')}
                  style={{
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={columns.length} className={`${styles.stateCell} text-body-m`}>
