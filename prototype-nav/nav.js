(function () {
  ...
  var STORAGE_KEY = 'nivelo.protoNav.state.v1';
  ...
  // Um item de `screens` pode ser uma tela de verdade OU um grupo `type:'flow'`
  // (fluxo secundário dentro da mesma jornada, ex: "Recuperar senha" agrupando
  // suas próprias telas) que por sua vez tem seu próprio `screens` aninhado —
  // por isso percorre recursivamente em vez de assumir só um nível.
  ...
  function buildScreenRow(journey, flow, screen) {
    ...
    // Clique na linha inteira navega (não só no texto do label) — a
    // miniatura é o alvo mais óbvio de clique e antes não fazia nada.
  ...
  function selectScreen(id) {
    ...
    if (currentAttr && currentPath === nextPath) {
      ...
      // `location.hash` uma vez, no load) nunca roda de novo pro novo
  ...
  function setDevice(device) {
    ...
    // Bug real corrigido: trocar de device chamava `selectScreen()` de
    // novo, que (pra telas sem `mobile` próprio, ou seja quase todas —
    // ver nota no topo de nav.config.js) via o MESMO arquivo pros dois
    // caminhos e disparava o truque de "about:blank + requestAnimationFrame"
    // (pensado só pra troca de variante via hash, que precisa de reload de
    // verdade). Esse truque depende do navegador disparar o rAF a tempo —
    // se não disparar (ex.: durante o próprio clique/resize que trocou o
    // device), a prévia ficava travada em `about:blank`, parecendo que
    // "saiu" da tela (reportado acontecendo ao trocar pra Mobile estando em
    // Cadastro). Como o tamanho do iframe já é 100% CSS (`data-device`, ver
    // nav.css), trocar de device nunca precisa de navegação nenhuma quando
    // o arquivo de destino é o mesmo — só recarrega de verdade se o `src`
    // resolvido realmente mudar (telas que um dia ganhem `mobile` próprio).
    if (state.screenId) {
  ...
  // Único controle de verdade da flag: as telas do produto (`dev-notes.js`)
  // só leem `localStorage`, nunca mostram um switch nelas mesmas — o
  // protótipo tem que ficar limpo por padrão pra demonstração ao cliente.
  // As miniaturas/preview (iframes da mesma origem) reagem sozinhas via
  // evento `storage`, sem precisar recarregar nada aqui.
  ...
  els.searchInput.addEventListener('input', function () {
    ...
    els.tree.querySelectorAll('.pn-journey').forEach(function (journeyWrap) {
      ...
      // Grupos de fluxo (ex: "Recuperar senha") só existem entre a jornada e
      // a tela — precisam do mesmo tratamento de mostrar/esconder/expandir
      // com base em alguma tela filha (em qualquer nível) ter dado match.
